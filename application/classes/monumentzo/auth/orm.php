 ...
 */
class Monumentzo_Auth_ORM extends Auth {
	...
	protected function _login($user, $password, $remember)
	{
		...
		if ($user->has('Role', ORM::factory('Role', array('Name' => 'login'))) AND $user->HashedPassword === $password)
		{
			if ($remember === TRUE)
			{
				// Token data
	...
	public function logout($destroy = FALSE, $logout_all = FALSE)
	{
		...
		if ($token = Cookie::get('authautologin'))
		{
			...
			{
				ORM::factory('User_Token')->where('UserID', '=', $token->UserID)->delete_all();
			}
			elseif ($token->loaded())
			{
				$token->delete();
	...
	public function check_password($password)
	{
		...
		return ($this->hash($password) === $user->HashedPassword);
	}
